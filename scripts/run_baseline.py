...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Time before a reference event that is labeled positive.",
    )
    parser.add_argument(
        "--post-event-exclusion-s",
        type=float,
        default=1.0,
        help="Time after a reference event excluded from labels/evaluation.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Model score threshold used to print the main fold result.",
    )
    parser.add_argument(
        "--threshold-sweep",
        type=str,
        default="0.05,0.10,0.15,0.20,0.30,0.50",
        help="Comma-separated score thresholds used for trade-off inspection.",
    )
    parser.add_argument(
        "--max-early-s",
        type=float,
        default=2.0,
        help="Maximum allowed advance time for matching an alarm to an event.",
    )
    parser.add_argument(
        "--refractory-s",
        type=float,
        default=1.0,
        help="Minimum time between repeated alarms in the same run.",
    )
    parser.add_argument(
        "--n-splits",
        type=int,
        default=5,
        help="Number of GroupKFold splits by run name.",
    )
    parser.add_argument(
        "--n-estimators",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional maximum tree depth for edge-aware forests.",
    )
    parser.add_argument(
        "--min-samples-leaf",
        type=int,
        default=1,
        help="Minimum samples per tree leaf.",
    )
    parser.add_argument(
        "--random-state",
        type=int,
        ...
        for episode in episodes:
            windows.extend(
                make_sliding_windows(
                    ...
                    end_time=episode.extended_end_time,
                )
            )

        evaluated_duration_by_run[run_name] = sum(
            episode.extended_duration_s for episode in episodes
        )
